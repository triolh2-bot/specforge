...
def generate_markdown_export(analysis: dict[str, Any]) -> tuple[str, str]:
    ...
    content = "\n".join(lines)
    filename = f"prd-{analysis.get('domain', 'project')}-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M')}.md"
...
<footer>Generated by SpecForge &middot; specforge.dev</footer>
    ...
    filename = f"prd-{analysis.get('domain', 'project')}-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M')}.html"
...
def generate_json_export(analysis: dict[str, Any]) -> tuple[str, str]:
    ...
    filename = f"prd-{analysis.get('domain', 'project')}-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M')}.json"
    return content, filename
