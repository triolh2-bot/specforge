...
- `GET /health/ready` checks the database, job queue state, and MiniMax provider configuration.
...
- Provider error rate panel for MiniMax auth, token exchange, and API request failures.
...
- Warn on sustained MiniMax authentication failures or provider request errors.
...
4. If provider status is degraded, confirm MiniMax API key or OAuth configuration and token freshness.
